...
class Client{
	 ...
	 *
	 */
	function Client($url,$serialNumber,$password,$sessionKey='',$proxyhost = false,$proxyport = false,$proxyusername
	= false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $contentTpl)
	{
		$this->url = $url;
	 ...
	 * 将验证码套进内容模板
	 * @param $code
	 * @return mixed 返回将要发送的内容
	 */
	function getContent($code = ''){
		if($code == ''){
			$code = $this->generateKey();
		}
		$content = str_replace('[CODE]',$code,$this->contentTpl);
